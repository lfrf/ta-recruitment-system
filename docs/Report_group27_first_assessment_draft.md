...
The team then transformed the requirements into a product backlog using Agile user stories. Related requirements were grouped into epics such as **Applicant Profile and Application**, **Vacancy Posting and Applicant Review**, **Admin Workload Monitoring**, and **System Quality and Data Reliability**. This made the backlog easier to prioritise and gave a clearer structure for iteration planning.

The product backlog was not produced by a single member in isolation. Instead, the team used the requirements findings as a shared input and co-created the user stories through discussion. Yuzhang Wu contributed the stakeholder evidence and proposed applicant- and admin-related stories based on the findings. Chensiyuan Qing helped structure the backlog and rewrote raw requirements into standard user story format. Fuhe Huang checked whether the stories matched the interface flow and prototype direction. Mu Du added validation, reliability, and workload-related concerns from an implementation and testing perspective. Tao Li refined wording for clarity and consistency with the report. Yifu Feng reviewed scope, priority, and overall fit with the project plan.

Each story was written using the form **“As a…, I want…, so that…”** so that the backlog expressed user value rather than only listing technical functions. For example, the applicant profile requirement was rewritten as: an applicant wants to maintain a reusable profile so that repeated data entry is avoided. Likewise, the vacancy review requirement was expressed as a module organiser wanting to see applicant profile and CV information together so that candidate review is more efficient.

This approach improves traceability from requirements to backlog items. It also provides stronger Agile evidence, because the backlog can be shown as the result of team interpretation and refinement rather than a single-person document. The final backlog therefore captures both stakeholder needs and collaborative prioritisation.
